...
class ShortUrlsStorageUpdateTesCase(TestCase):
    def setUp(self) -> None:
...
class ShortUrlsStorageGetShortUrlTesCase(TestCase):
    SHORT_URL_COUNT = 3
    ...
    def test_get_list(self) -> None:
        ...
        expected_slug = {su.slug for su in self.short_urls}
    ...
    def test_get_by_slug(self) -> None:
        for short_url in self.short_urls:
            with self.subTest(
                slug=short_url.slug,
                msg=f"Validate can get slug {short_url}!r",
            ):
...
def test_create_or_raise_if_exists(short_url: ShortUrl) -> None:
    ...
    assert exp_info.value.args[0] == short_url_create.slug
